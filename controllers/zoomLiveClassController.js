...
exports.createLiveClass = async (req, res, next) => {
  try {
    ...
    for (const id of section_id) {
      ...
      if (class_sec) class_sections.push(class_sec.getDataValue("id"));
    }

    let created_by;

    if (req.user) created_by = req.user.id;
...
exports.deleteLiveClass = async (req, res, next) => {
  try {
    ...
    await zoomLiveClassSection.destroy({
      where: { zoom_live_id: req.params.id },
    });
