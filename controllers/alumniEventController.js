...
exports.createAlumniEvent = async (req, res, next) => {
  try {
    upload(req, res, async (err) => {
      ...
      if (req.file) req.body.photo = `/public/alumniEvent/${req.file.filename}`;
...
exports.deleteAlumniEvent = async (req, res) => {
  try {
...
exports.getAllAlumni = async (req, res, next) => {
  try {
    ...
    if (req.query.section_id) whereObj.section_id = req.query.section_id;
    ...
    for (const student of students) {
      ...
      let isPresent = await manageAlumni.findOne({
...
exports.updateAlumni = async (req, res, next) => {
  try {
    uploadAlumni(req, res, async (err) => {
      if (req.file) req.body.current_photo = `/public/${req.file.filename}`;
      ...
      if (alreadyExists)
        await manageAlumni.update(req.body, {
          where: { id: alreadyExists.getDataValue("id") },
        });
      else {
...
exports.deleteAlumni = async (req, res, next) => {
  try {
    await manageAlumni.destroy({ where: { student_id: req.params.id } });
    res.status(200).json({
      status: "success",
